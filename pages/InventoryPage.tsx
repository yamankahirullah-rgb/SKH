...
const InventoryPage: React.FC = () => {
  ...
  const handleTransferItemChange = (index: number, field: keyof StockItem, value: string | number) => {
          ...
          <tbody className="divide-y">
            {filteredMaterials.map((material: Material) => {
             ...
             {filteredMaterials.length === 0 && (
                <tr>
                    <td colSpan={warehouses.length + 4} className="text-center p-4 text-gray-500">لا توجد مواد تطابق البحث.</td>
                </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="space-y-4">
            ...
            {transferItems.map((item, index) => (
                <div key={index} className="flex items-center gap-2">
                    ...
                    <button onClick={() => removeTransferItem(index)} className="text-red-500 hover:text-red-700 p-1">
                        <MinusCircle size={20} />
                    </button>
                </div>
            ))}
            <button onClick={addTransferItem} className="flex items-center text-red-600 mt-2">
